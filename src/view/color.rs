...
fn parse_hex_color(color: &str) -> Color {
    ...
    Color::Rgb(
        u8::from_str_radix(capture.get(1).unwrap().as_str(), 16).ok().unwrap(),
        u8::from_str_radix(capture.get(2).unwrap().as_str(), 16).ok().unwrap(),
        u8::from_str_radix(capture.get(3).unwrap().as_str(), 16).ok().unwrap(),
    )
}
